  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        ...
        >
          <div className={`p-8 text-center ${isLastWarning ? 'bg-red-50' : 'bg-amber-50'}`}>
            ...
            <h2 className={`text-2xl font-black mb-2 ${isLastWarning ? 'text-red-700' : 'text-amber-700'}`}>
          ...
          <div className="p-8 space-y-6">
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
              <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Warning Status</span>
              <div className="flex gap-1.5">
                {[...Array(maxViolations + 1)].map((_, i) => (
                  <div 
                    key={i} 
                    className={`w-8 h-2 rounded-full transition-all duration-500 ${
                      i < violationCount ? 'bg-red-500' : 'bg-slate-200'
                    }`}
                  />
                ))}
              </div>
            ...
            <div className="text-center">
              <p className="text-sm font-bold text-slate-600 mb-6 leading-relaxed">
                You have used <span className="text-red-600 font-black">{violationCount}</span> of <span className="text-slate-900 font-black">{maxViolations + 1}</span> chances. 
